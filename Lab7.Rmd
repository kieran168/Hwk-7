...
proppred <- prop.table(RFpredtable)
# proppred
...
The optimal mtry number is 5, which in concert with a smaller number of trees reduced the Out of Box error rate from 13.77% to `r (best.m[2])*100`%. This rate tells us that as the model makes the splits and performs prediction tests on internally designated training and test data, it makes errors at a rate of `r (best.m[2])*100`%, performing slightly better than when the model is applied to the overall test data. 
